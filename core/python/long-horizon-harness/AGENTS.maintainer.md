# Agent Guide — Horizon (long-horizon ADK agent, package `horizon`)

Self-improving long-horizon agent on Google ADK + Vertex AI. Read before non-trivial changes: **`docs/architecture.md`** (overview) and `docs/{sandbox-lifecycle,memory,security-model,routines}.md` (subsystems; `security-model.md` is the per-layer auth and security model).

This repo: `<repo-root>` (the checkout root; paths below are relative to it).

**How these docs relate.** This file (AGENTS.md) is the maintainer view — conventions, the architecture map, and internals. The canonical reader-facing references are [`docs/configuration.md`](docs/configuration.md) (env vars), [`docs/extending.md`](docs/extending.md) (extension points), [`docs/architecture.md`](docs/architecture.md) (the map), [`docs/commands.md`](docs/commands.md) (slash commands), and [`docs/security-model.md`](docs/security-model.md) (per-layer security). Don't duplicate those here — link to them.
...
5. **Respect the scope table.** Out-of-scope: alternate gateways (Telegram/Discord/Slack/WhatsApp/Signal), sandbox backends other than Agent Runtime + local fallback, browser/computer-use/image-gen/video-gen/TTS/voice tools, MCP server, TUI, ACP.
...
├── examples/                     # minimal_agent.py — smallest runnable wiring (env + uvicorn examples.minimal_agent:app)
├── scripts/
└── tests/
